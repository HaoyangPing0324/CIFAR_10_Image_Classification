...
@Time    : 2025/12/14
@Desc    : 使用 PyTorch 通过全连接神经网络(FCN)进行 CIFAR-10 图像分类
...
class MLP(torch.nn.Module):
    def __init__(self, n_input=32*32*3, n_hidden=[512,256,128], n_output=10):
        super(MLP, self).__init__()

        layers = []
        ## 需要包含至少 3 层全连接层
        total_fc_layers = len(n_hidden) + 1
        if total_fc_layers < 3:
            raise ValueError(f"全连接层数量需至少为3层，当前为{total_fc_layers}层（隐藏层{len(n_hidden)}层 + 输出层1层）")
...
def main():
    ...
    model = MLP(n_hidden=n_hidden).to(device)                                   # 模型构建
